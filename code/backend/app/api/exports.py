...
def create_export_endpoint(project_id: str, payload: CreateExportRequest, db: Session = Depends(get_db)):
    try:
...
def get_export_endpoint(project_id: str, export_id: str, db: Session = Depends(get_db)):
    try:
        require_project(project_id)
    except KeyError:
        raise api_error(404, "project_not_found", "Project not found")
