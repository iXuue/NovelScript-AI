...
from app.services.conversation_service import (
    confirm_feedback_plan,
    create_feedback_plan,
    create_scene_plan_confirmation_guidance,
    list_primary_messages,
    send_message,
)
...
@router.post("/projects/{project_id}/conversations/primary/scene-plan-confirmation-guidance")
def scene_plan_confirmation_guidance_endpoint(
    project_id: str,
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user),
):
    try:
        require_project(project_id, db, current_user.user_id)
        return create_scene_plan_confirmation_guidance(project_id, db)
    except KeyError:
        raise api_error(404, "project_not_found", "Project not found")
