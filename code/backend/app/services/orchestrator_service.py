...
def generate_script(project_id: str, db=None, llm_provider: LLMProvider | None = None) -> dict:
    ...
    for scene_index, scene in enumerate(scene_plan["scenes"], start=1):
        ...
        evidence_map[block_id] = {
            ...
            "evidence": [
                {
                    "source_paragraph_id": paragraph.paragraph_id if paragraph else (source_paragraph_ids[0] if source_paragraph_ids else None),
                    "source_evidence_id": None,
                    "chapter_id": scene["source_chapter_ids"][0],
                    "paragraph_id": paragraph.paragraph_id if paragraph else (source_paragraph_ids[0] if source_paragraph_ids else ""),
                    "text": text,
                }
            ],
        }
